...
void RenderContextVK::operator()(const cmd::CreateTexture2D& c) {
    ...
    createImage(static_cast<u32>(c.width), static_cast<u32>(c.height), format,
    ...
    // Create image view.
    texture.image_view = createImageView(texture.image, format, vk::ImageAspectFlagBits::eColor);
...
void RenderContextVK::createSwapChain() {
    ...
    for (const auto& swap_chain_image : swap_chain_images_) {
        swap_chain_image_views_.push_back(createImageView(
            swap_chain_image, swap_chain_image_format_, vk::ImageAspectFlagBits::eColor));
    }

    // Depth.
    depth_format_ = vk::Format::eD32Sfloat;
    createImage(swap_chain_extent_.width, swap_chain_extent_.height, depth_format_,
                vk::ImageTiling::eOptimal, vk::ImageUsageFlagBits::eDepthStencilAttachment,
                vk::MemoryPropertyFlagBits::eDeviceLocal, depth_image_, depth_image_memory_);
    depth_image_view_ = createImageView(depth_image_, depth_format_, vk::ImageAspectFlagBits::eDepth);
}
...
vk::Image RenderContextVK::createImage(u32 width, u32 height, vk::Format format,
